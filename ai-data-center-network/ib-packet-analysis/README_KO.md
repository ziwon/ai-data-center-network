...
| RDMA READ | 참조 모델 전용 | 미래 캡처를 위해 `BTH + RETH` 요청과 응답 패킷 동작 설명용으로 추가 |
| RDMA WRITE | 참조 모델 전용 | 미래 캡처를 위해 `BTH + RETH + payload` 요청 동작 설명용으로 추가 |
...
| 비트 단위 패킷 포맷 레퍼런스 | 동반 문서 | [`packet-format-reference_KO.md`](packet-format-reference_KO.md) — LRH/GRH/BTH/extended header/MAD/SMP DR/IPoIB 비트 레이아웃과 BTH opcode 마스터 표 |

이 보고서를 읽을 때, 관측 섹션은 제공된 pcap 파일의 분석으로, RDMA READ/WRITE 섹션은 일방향(one-sided) RDMA 동작을 포함하는 미래 캡처를 위한 패킷 분석 가이드로 다루세요. 보고서가 참조는 하지만 빠짐없이 표로 정리하지 않은 바이트/비트 수준 필드 레이아웃은 [동반 패킷 포맷 레퍼런스](packet-format-reference_KO.md)를 참고하세요.
...
- **패킷 구조 관점**: 개별 패킷이 라우팅 헤더, 전송 헤더, 선택적 확장 헤더, 페이로드, 무결성 검사 포함해 와이어상에서 어떻게 인코딩되는지.
...
이 캡처들은 외부 래퍼로 Endace **Extensible Record Format** (ERF)을 사용합니다. 와이어상의 InfiniBand 프레임은 캡처 장비가 발행한 ERF 레코드로 캡슐화되며, `tshark`는 이 외부 레코드를 디코드한 다음 내부 바이트를 InfiniBand dissector에 넘깁니다. ERF가 보존하는 것과 숨기는 것을 이해하는 것이 "패킷이 보인다"와 "스니퍼가 정확히 무엇을 기록했는지 안다"를 가르는 차이입니다.
...
| Truncated | `erf.flags.trunc` | `0` | 와이어 길이 그대로 캡처됨 |
...
| Wire length | `erf.wlen` | `114` | 원래 와이어 바이트 수 |
...
### 워크드 예제: ERF + IB 프레임 레이아웃

다음은 `infiniband.pcap` frame 10(IPoIB ICMP echo request를 운반하는 RC SEND Only)의 익명화된 레이아웃입니다. ERF 외부 레코드, InfiniBand 헤더, EtherType-캡슐화된 IPoIB 페이로드, 후행 CRC가 단일 114바이트 와이어 프레임에 어떻게 공존하는지 보여줍니다.

```text
Frame 10 — 와이어 114바이트 / ERF 레코드 136바이트 / capture interface 1 (Port B)
...
> 여기 나열된 모든 헤더(LRH, GRH, BTH, DETH, RETH, AETH, AtomicETH, ImmDt, IETH, RDETH, XRCETH, MAD, SMP DR, IPoIB encap)의 비트 단위 필드 레이아웃, AETH syndrome 인코딩, BTH opcode 마스터 표는 동반 문서 [`packet-format-reference_KO.md`](packet-format-reference_KO.md)를 참고하세요. 이 섹션은 상위 수준 패킷 모양을 제공하며, 레퍼런스 문서는 바이트와 비트 경계까지 파고듭니다.
...
flowchart TB
    ...
    RDMA_WRITE["RDMA WRITE<br/>일방향 push, 본 캡처에는 미수록"]
    RDMA_WRITE_SEQ["LRH -> BTH -> RETH -> data payload"]
    RDMA_READ["RDMA READ<br/>일방향 pull, 본 캡처에는 미수록"]
...
현재 pcap 세트는 완전한 RDMA READ나 RDMA WRITE 교환을 **포함하지 않습니다**. 따라서 이 섹션은 미래 캡처에 일방향 RDMA 동작이 포함될 경우 그러한 패킷을 어떻게 해석해야 하는지 설명하는 참조 모델입니다. 공식 자료와 참고문헌의 Tencent Cloud 글에서 설명하는 InfiniBand 전송 계층 동작에 기반합니다.

일방향 RDMA 동작의 핵심 헤더는 RDMA Extended Transport Header인 `RETH`입니다.

| 헤더 | 중요 필드 | 왜 중요한가 |
| --- | --- | --- |
| `BTH` | opcode, 목적지 QP, PSN, ACK request | 동작 타입과 패킷 순서 식별 |
| `RETH` | 가상 주소, `rkey`, DMA length | 원격 메모리 범위를 인가하고 기술 |
| `AETH` | ACK/NAK syndrome, MSN | 신뢰 전송 진행을 확인하거나 오류 보고 |
| Payload | read 응답 데이터 또는 write 데이터 | 동작 방향에 따른 사용자 데이터 운반 |

### 전송 서비스별 동작 지원

InfiniBand 전송 서비스는 verbs 스타일의 모든 동작을 동등하게 지원하지 않습니다. 실용적 결론은 응답이 필요하거나 엄격한 순서가 필요하거나 read-modify-write 의미를 갖는 일방향 동작은 신뢰 전송 컨텍스트가 필요하다는 것입니다.
...
현대 RDMA 소프트웨어에서는 `RC`가 RDMA READ와 Atomic 동작에 가장 흔히 쓰이는 실용 전송입니다. `RD`도 InfiniBand 아키텍처에서 이를 지원하지만 주류 애플리케이션 스택에서 기본 선택이 되는 경우는 드뭅니다.
...
sequenceDiagram
    ...
    Resp->>Resp: rkey, VA, length, 순서, responder 자원 검증
    Resp-->>Req: READ Response<br/>데이터를 requester로 반환
```

RDMA READ는 단방향 패킷이 아닙니다. responder 측에 작업을 발생시킵니다 — responder RNIC이 요청을 검증하고, 원격 메모리를 가져오고, 하나 이상의 응답 패킷을 생성하고, 순서를 유지하고, 재시도/오류를 처리해야 합니다. `UC`에는 신뢰 응답/ACK 머신이 없고, `UD`는 원격 메모리 읽기에 필요한 연결된 responder 상태가 없는 메시지 지향 데이터그램 전송입니다.
...
Atomic 동작은 원격 메모리 위치에서 단일 전역 순서 read-modify-write를 요구합니다. requester는 또한 반환 값과 동작 완료 여부를 알기 위해 신뢰 응답이 필요합니다. 이는 연결 상태, 순서, 재시도/오류 의미를 요구하므로, 실용적 배포는 atomic에 RC 스타일 신뢰 전송을 사용합니다.

> NCCL, UCX, MPI, DC 전송에 대한 실용 노트:
>
> AllReduce 같은 NCCL collective는 대용량 청크를 GPU 메모리에서 다른 GPU 메모리로 이동시킵니다. 일부 단계는 push 스타일 전송으로 구현 가능하지만, pull 기반 peer access 패턴은 RDMA READ 의미에서 이득을 얻습니다. Ring과 tree 알고리즘도 예측 가능한 순서와 완료 동작이 필요하므로 신뢰 전송이 중요합니다.
>
> UCX는 범용 통신 계층입니다. 작은 메시지는 SEND/RECV 또는 inline 경로를 사용하고, 큰 메시지는 RDMA를 사용할 수 있습니다. UCX는 또한 tag matching과 RMA 스타일 동작(가능한 전송에서의 atomic 포함)을 노출합니다. 이는 자연스럽게 READ, Atomic, 순서, 재시도 의미가 필요한 경로에 신뢰 연결 지향 전송을 선호하게 합니다.
>
> MPI 구현은 종종 `MPI_Put`, `MPI_Get`, `MPI_Accumulate` 같은 일방향 프리미티브를 전송이 지원할 때 RDMA WRITE, RDMA READ, Atomic 동작에 매핑합니다. MPI 의미가 신뢰 통신을 가정하므로 기반 네트워크 경로는 보통 신뢰 완료와 순서 동작이 필요합니다.
>
> 대규모 클러스터 스케일에서, 순수 RC는 비싸질 수 있습니다 — 조밀한 all-to-all peer mesh가 많은 QP와 연관 HCA 메모리를 요구할 수 있기 때문입니다. `DC` 전송, 즉 Dynamically Connected transport가 신뢰 의미를 유지하면서 연결 자원을 동적으로 재사용하여 이를 해결합니다. 그래서 DC 스타일 전송이 대규모 InfiniBand 배포에서 중요합니다. NVIDIA SHARP와 NCCL-RDMA-SHARP 경로도 현대 collective 스택에 나타날 수 있지만, DC, UCX, verbs, SHARP의 정확한 사용은 하드웨어, 플러그인 가용성, 토폴로지, 런타임 환경 설정에 따라 달라집니다.

### 전송 계층에서 확인할 가치가 있는 디테일

Tencent Cloud 글이 유용한 이유는 RDMA READ/WRITE를 verbs API 호출이 아닌 InfiniBand 전송 계층 동작으로 설정하기 때문입니다. 다음 디테일은 패킷 분석으로 가져갈 가치가 있습니다:
...
| `PSN`은 단순 카운터가 아님 | Packet Sequence Number는 responder/requester가 누락, 중복, 순서 어긋남 패킷을 감지하는 데 사용됩니다. 신뢰 서비스에서는 ACK/NAK와 재시도 동작을 주도합니다. |
| `P_Key`와 목적지 QP는 검증 입력 | 패킷의 목적지 QP, QP 상태, 전송 타입, 파티션 키가 responder 컨텍스트와 맞지 않으면 조용히 드롭될 수 있습니다. |
| `RETH`는 보호 경계 | `RETH`는 단순한 주소 디스크립터가 아닙니다. responder는 원격 메모리에 접근하기 전에 `rkey`, 접근 권한, 가상 주소 범위, DMA length를 검증해야 합니다. |
| `AETH`는 ACK/NAK 상태 운반 | 신뢰 전송에서 `AETH`는 진행이 ACK되었는지, 재시도/오류 조건이 있는지를 requester에게 알립니다. |
| `ICRC/VCRC`는 와이어상 무결성 검사 | 캡처 도구는 일부만 노출할 수 있으며, 무효 CRC는 보통 유용한 전송 계층 해석 전에 폐기됩니다. |

`P_Key`는 특별한 주의를 받을 만합니다. `BTH`에 실리는 파티션 멤버십 값으로, 정신적으로는 fabric 수준의 테넌트나 격리 태그와 비슷합니다. 상위 비트는 full vs limited 멤버십을 표시하고 하위 비트는 파티션을 식별합니다. 패킷의 `P_Key`가 목적지 포트의 파티션 멤버십이나 QP 컨텍스트와 일치하지 않으면 해당 파티션의 유효 트래픽으로 받아들여지지 않습니다. 그래서 패킷 드롭을 디버그할 때 `P_Key`는 목적지 QP, 전송 타입, QP 상태와 함께 읽어야 합니다.

놓치기 쉬운 두 가지 미묘한 포인트:

- 다중 패킷 SEND와 RDMA WRITE 메시지는 해당 메시지의 마지막 패킷이 생성될 때까지 동일 send queue의 다른 동작과 인터리빙되지 않습니다.
- RDMA READ는 다르게 동작합니다: READ 요청을 발행한 후 requester는 READ 응답을 기다리지 않고 후속 요청을 발행할 수 있지만, outstanding READ와 ATOMIC 동작의 최대 개수는 연결 셋업 중 협상됩니다.

### RDMA READ

RDMA READ는 일방향 **pull**입니다. requester가 responder RNIC에게 원격 메모리에서 읽어 데이터를 반환해 달라고 요청합니다.
...
sequenceDiagram
    ...
    Req->>LocalMR: requester 로컬 버퍼에 payload DMA 쓰기
```
...
- responder는 재시도 요청, `rkey`, 원격 가상 주소, 접근 권한을 검증합니다.
- requester는 협상된 연결 한도에 따라 둘 이상의 outstanding READ를 가질 수 있습니다.
...
요청 디코드는 일방향 READ의 핵심 증거입니다: `BTH + RETH`가 있고, `RETH`가 원격 가상 주소, `rkey`, 요청된 DMA length를 운반합니다. `BTH`는 또한 `Partition Key`(`P_Key`)를 운반하며, 이는 패킷이 사용하는 InfiniBand 파티션 멤버십을 식별합니다. 흔히 보이는 `0xffff` 같은 값은 기본 파티션의 full membership을 나타내지만, 프로덕션 fabric은 격리를 위해 다른 파티션 키를 사용할 수 있습니다. response-middle 디코드는 역방향 데이터 이동을 보여줍니다: 데이터 페이로드를 운반하지만 `RETH`도 `AETH`도 없습니다. 이는 다중 패킷 READ 모델과 일치합니다 — `AETH`는 first, last, only 응답 패킷에 나타나고, middle 응답 패킷은 데이터만 운반하는 세그먼트입니다.
...
`AETH`는 신뢰 전송의 핵심 ACK/NAK 운반자입니다. 정상 ACK는 responder가 관련 신뢰 동작의 진행을 받아들였음을 표시합니다. syndrome이 NAK나 오류 조건을 표시하면, requester는 전송 상태와 재시도 카운터에 따라 Work Request를 재시도하거나 실패 처리해야 할 수 있습니다. 패킷 분석에서 `BTH`는 이것이 RC acknowledge 패킷이고 어느 파티션/QP 컨텍스트에 속하는지 알려주며, `AETH`는 성공 ack인지 오류/흐름 제어 신호인지 알려줍니다.

### RDMA WRITE

RDMA WRITE는 일방향 **push**입니다. requester가 responder가 이미 등록하고 메타데이터 교환을 통해 공유한 원격 메모리 범위로 데이터를 보냅니다.
...
| 페이로드 방향 | Responder → requester | Requester → responder |
| AETH | First, last, only read 응답 | ACK/NAK 응답 |
| Target CPU 관여 | 데이터 경로에 없음 | 데이터 경로에 없음 |

이는 현재 패킷 세트가 RDMA/IB 기초에는 유용하지만 완전한 일방향 RDMA 데이터 경로 캡처로는 다룰 수 없는 이유를 설명합니다.
...
- RDMA 의미와 혼동하면 안 됩니다 — IPoIB는 일방향 RDMA와 같지 않습니다.
...
- AETH가 ACK 패킷에 보입니다.
- 완전한 RDMA Write나 Read 예시는 아니지만, 1장의 양방향 신뢰 전송 논의에 가깝습니다.
...
- 완전한 일방향 RDMA 데이터 이동 예시.
- NCCL collective 트래픽.

따라서 올바른 해석은:

> 이 캡처들은 InfiniBand fabric 관리, IPoIB, 일부 reliable connection 전송 동작을 보여줍니다. 1장의 RDMA/IB 개념을 뒷받침하지만 RDMA Read나 RDMA Write 데이터 이동을 완전히 시연하지는 않습니다.

위의 [RDMA Read/Write 패킷 분석 모델](#rdma-readwrite-패킷-분석-모델) 섹션은 일방향 동작을 포함하는 미래 캡처에서 무엇이 보여야 하는지 설명합니다.
...
미래 캡처에 RDMA READ나 WRITE 패킷이 포함된 경우, BTH opcode부터 시작해 RETH/AETH 디테일로 확장:
...
6. **데이터 경로/제어 경로 구분은 명시적으로 유지되어야 합니다.**
   관리 캡처는 대부분 제어 경로입니다. IPoIB와 RC SEND/ACK는 데이터 경로에 더 가깝습니다. 완전한 RDMA Read/Write는 RETH와 일방향 동작 필드를 포함하는 추가 캡처가 필요합니다.
