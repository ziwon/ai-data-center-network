# InfiniBand 패킷 포맷 레퍼런스

[English](packet-format-reference.md) | **한국어**

이 문서는 InfiniBand 패킷 헤더의 비트 단위 레퍼런스이며, [메인 분석 보고서](README_KO.md)의 동반 문서로 의도되었습니다. 보고서는 `ib-packets` 데이터셋에서 무엇이 관측되었는지에 초점을 두고, 이 레퍼런스는 모든 IB 패킷 헤더가 와이어상에서 어떻게 보이는지에 초점을 둡니다. 구체적 예시가 있는 곳에서는 보고서의 프레임 수준 증거로 anchor 링크를 제공합니다.

이 레퍼런스를 사용하는 경우:

- hex dump를 읽고 필드 경계를 식별할 때
- 주어진 BTH opcode 다음에 어떤 extended header가 와야 하는지 검증할 때
...
와이어상의 IB 패킷은 헤더, 페이로드, CRC의 엄격한 연결입니다. 어떤 extended header가 어떤 순서로 나타나는지는 `LRH.LNH`(GRH 존재 여부)와 BTH opcode(어떤 extended header가 따르는지)로 완전히 결정됩니다.
...
- extended header의 종류와 순서는 [Operation → Extended Header 매핑](#operation--extended-header-매핑)을 따릅니다.
- `ICRC`와 `VCRC`는 항상 와이어상에서 패킷을 마무리합니다.
...
구체적 예시: 이 데이터셋의 모든 패킷은 `LNH = 0x2`를 가지므로 GRH가 디코드되지 않습니다. `infiniband.pcap` frame 10의 워크드 예제는 메인 보고서의 [ERF 캡처 해부](README_KO.md#erf-캡처-해부) 섹션을 참고하세요.
...
이 데이터셋은 GRH를 가진 패킷을 포함하지 않으므로, 이 섹션은 미래의 서브넷 간 캡처를 위한 순수 레퍼런스입니다.
...
| SE | Solicited Event — responder의 CQ 이벤트를 트리거해야 하는 SEND 또는 RDMA WRITE 메시지의 마지막 패킷에 설정 |
...
| AckReq | RC 트래픽에서 설정 시 responder가 ACK를 생성해야 함 |
...
BTH 다음에 어떤 extended header가 따르는지는 opcode가 완전히 결정합니다. IBA 사양은 이를 opcode별 표로 인코딩합니다 — 동작별 요약은 [Operation → Extended Header 매핑](#operation--extended-header-매핑)에 있습니다.
...
responder는 `R_Key`에 등록된 MR과 비교해 요청을 검증해야 합니다: VA는 MR 주소 범위 안에 있어야 하고, `[VA, VA + DMALen)`은 경계 안이어야 하며, MR의 접근 권한이 필요한 READ 또는 WRITE를 포함해야 합니다.
...
구체적 예시: `infiniband.pcap` frame 11은 `Syndrome = 31` 십진수 = `0x1F` = `0 00 11111`입니다. 이는 `OpCode = 00 (ACK), Value = 11111 (credit 정보 없음)`으로 디코드됩니다 — 흐름 제어 힌트가 없는 정상 acknowledgment. 컨텍스트는 [메인 보고서의 frame-11 매핑](README_KO.md#infinibandpcap)을 참고하세요.
...
원래(atomic 이전) 값을 requester에 다시 운반합니다. `ATOMIC Acknowledge` 패킷에서 AETH 다음에 위치합니다.
...
responder에서 무효화할 R_Key를 운반합니다. `SEND Last with Invalidate`와 `SEND Only with Invalidate`에 존재합니다.
...
구체적 예시: `infiniband.pcap` frame 10은 `EtherType=0x0800, Reserved=0x0000`을 가지며 그 다음에 IPv4 ICMP Echo request가 옵니다. 메인 보고서의 [워크드 예제](README_KO.md#erf-캡처-해부)를 참고하세요.
...
ERF 캡처에서 두 CRC 모두 필터 가능 필드(`infiniband.invariant.crc`와 `infiniband.variant.crc`)로 노출됩니다. Wireshark dissector는 검증하지 않으니 대신 `erf.flags.rxe`를 신뢰하세요. 메인 보고서의 [보존 매트릭스](README_KO.md#erf가-보존하는-것-vs-숨기는-것)를 참고하세요.
...
BTH 다음에 어떤 extended header가 나타나는지는 opcode가 결정합니다. hex dump를 읽고 "다음에 무엇이 오나?"를 물을 때 이 표를 사용하세요.
